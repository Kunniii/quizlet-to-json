  ...
  if (!req.query.url) {
    ...
    axios
      ...
      .then((response) => {
        ...
        for (let i in rawQuestions) {
          let qc;
          if (!reverse) qc = { word: rawQuestions[i].word, definition: rawQuestions[i].definition };
          else qc = { definition: rawQuestions[i].word, word: rawQuestions[i].definition };
          questions.push(qc);
        }
        let numberOfQuestions = questions.length;
        let responseData = {
